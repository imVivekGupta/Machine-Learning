...
def get_base_delta(h,y,activation_fn):
	...
	if activation_fn=='tanh':
		...
		temp = np.array([i*(1-i) for i in h])			# softmax derivative
		delta = delta*temp
	return delta
...
def error_sample_part_b(sample,weights,activation_fn):
	h,y = [],[]
...
def backprop_part_b(activation_fn,eta,iterations = 1000):
	w1 = np.random.uniform(-1,1,(1+len(vector),100))/10		# random initialization of weights
	w2 = np.random.uniform(-1,1,(101,50))/10
	w3 = np.random.uniform(-1,1,(51,2))/10
	...
	for i in range(iterations):
		if i in error_checkpoints:					# compute error at defined check-points
			...
			insample_error.append(error_sample_part_b('in',[w1,w2,w3],activation_fn))
			outsample_error.append(error_sample_part_b('out',[w1,w2,w3],activation_fn))
		...
		if y==0:
			label = np.array([[0],[1]])    # first neuron is spam probability, second neuron is ham prob
		else:
			label = np.array([[1],[0]])
		...
		h = softmax(s3)						# h -> output of layer 3
		
		# backprop
		delta3 = get_base_delta(h,label,'softmax')			# get delta of final layer
		if activation_fn=='tanh':
			delta2 = (1-x2**2)*(w3.dot(delta3))			# compute delta2 from delta3
			delta1 = (1-x1**2)*(w2.dot(delta2[1:]))  #101 x 1	# compute delta1 from delta2
		else:
			delta2 = sigmoid(x2)*(1-sigmoid(x2))*(w3.dot(delta3))	# compute delta2 from delta3
...
def plot(insample_error,outsample_error):
	...
	plt.legend()
	plt.title('Error v/s iterations (Softmax B)')
	plt.savefig('./result_files/plot_B.png')
...
def stats_summary(weights,activation_fn,threshold=0.5):
	...
	for i in test_ohe:
		...
		if (h==1 and y==1):					# correctly classified spam (true positive)
			...
			fn +=1		
		else:							# incorrectly classified actual ham (false positive)
			fp +=1
...
f = open('./result_files/b_results.txt','w')
f.write('	Activation function: sigmoid (layer 1 and layer 2), softmax (layer 3)\n')
f.write('	Learning Rate (eta)	: 0.1\n')
f.write('	# Iterations: 50000\n')

print('starting backpropagation learning ...')
print('	Activation function	: sigmoid (layer 1 and layer 2), softmax (layer 3)')
print('	Learning Rate (eta)	: 0.1')
print('	#Iterations 		: 50000')

out = backprop_part_b('sigmoid',0.1,50000)			# run learning algorithm
